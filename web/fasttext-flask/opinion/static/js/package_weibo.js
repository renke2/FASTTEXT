...
function reposts_count_comparator(a, b) {
    return parseInt(b.reposts_count) - parseInt(a.reposts_count);
}
...
function sort_data(data, sort){
    ...
    if(sort == 'reposts_count'){
        data.sort(reposts_count_comparator);
    }
    return data;
}
...
function bindClusterSortClick(){
    ...
    $("#cluster_sort_by_attitudes_count").click(function(){
        ...
        global_select_subevent_sort = 'reposts_count';
        if (global_pie_data){
            refreshDrawCommentsOpinion(global_comments_opinion, global_select_subevent, global_select_subevent_sort);
        }
    });
...
var addition = 10;
var global_ajax_url = "/package_weibo/comments_list/?taskid=" + task_id + "&cluster_num=" + cluster_num + "&cluster_eva_min_size=" + cluster_eva_min_size + "&vsm=" + vsm + "&calcu=" + calc_label;
var sentiment_comments_pre = "/package_weibo/sentiment_comments?taskid=" + task_id + "&sort=";
var cluster_comments_pre = "/package_weibo/cluster_comments?taskid=" + task_id +"&sort=";
